...
def prediction(stock, n_days):
    print(f"Starting prediction for {stock} for {n_days} days.")
    
    # Fetch the data
    df = yf.download(stock, period='3mo')  # Change period to '3mo'
    if df.empty:
        raise ValueError(f"No data fetched for stock symbol {stock}.")
    ...
    if len(X) == 0 or len(Y) == 0:
        raise ValueError("The data for the given stock and period is empty.")
    ...
    fig.add_trace(go.Scatter(x=dates, y=best_svr.predict(output_days), mode='lines+markers', name='forecast'))
    fig.update_layout(title="Predicted Close Price of next " + str(n_days - 1) + " days", xaxis_title="Date", yaxis_title="Close Price")
